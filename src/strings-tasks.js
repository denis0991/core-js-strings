...
function removeFirstOccurrences(str, value) {
  ...
  if (str.indexOf(value) > 0) {
    const res1 = str.slice(0, str.indexOf(value));
    const res2 = str.slice(str.indexOf(value) + value.length);
    result = res1 + res2;
...
function removeLastOccurrences(str, value) {
  ...
  } else if (str.lastIndexOf(value) === 0) {
    result = str.slice(str.lastIndexOf(value) + value.length);
  } else {
    result = str;
  }
  return result;
...
function isPalindrome(str) {
  const cleanedStr = str.toLowerCase().replace(/ /g, '').replace(/\W/g, '');
